...
from .research_agent import LangChainResearchReportAgent  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(description="Agente de Pesquisa — LangChain")
    parser.add_argument("--topic", required=True, help="Tópico de pesquisa")
    args = parser.parse_args()

    agent = None
    try:
        agent = LangChainResearchReportAgent()
        result = agent.run_research_pipeline(args.topic)

        print("\n=== RELATÓRIO FINAL (LangChain) ===\n")
        print(result["report"])
